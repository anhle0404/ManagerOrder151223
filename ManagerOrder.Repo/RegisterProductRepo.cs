...
namespace ManagerOrder.Repo
{
    public class RegisterProductRepo:GenericRepo<RegisterProduct>
    {
        public object CheckValidate(RegisterProduct register)
        {
            ...
            {
                var listProducts = GetAll().Where(x => x.ProductCode.ToLower().Trim() == register.ProductCode.ToLower().Trim() && x.Id != register.Id).ToList();
            ...
            if (register.ProductImportPrice <= 0)
            {
                return new
                {
                    status = 0,
